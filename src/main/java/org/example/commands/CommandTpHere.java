...
public class CommandTpHere implements CommandExecutor
{
    ...
    public CommandTpHere (CheckPermission checkPermission, TeleportUtils teleportUtils, CommandRestrictionManager commandRestrictionManager)
    {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
    {
        ...
        if(args.length == 0)
        {
            player.sendMessage("Use /tphere <player>");
            return true;
        }
        ...
        teleportUtils.teleportToMe(player,target);
        player.sendMessage(target.getName() + " teleported to you!");
        target.sendMessage("You were teleported to " + player.getName());
